...
def register():
    ...
    if existing:
        ...
        if fields.get("Blocked"):
            return jsonify({"expired": True, "message": "Your access has been suspended. Contact us to continue."})
        ...
        if expires_str:
            try:
                ...
                if today > expires_date:
                    return jsonify({"expired": True, "message": f"Your {TRIAL_DAYS}-day trial has ended. Contact us to continue."})
            except ValueError:
                pass
...
def _airtable_find_prospect(email):
    ...
    params = {"filterByFormula": f"LOWER({{Email}})='{email.lower()}'"}
    try:
